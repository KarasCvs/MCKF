...
class MCUKF():
    ...
    def estimate(self, x_prior, sensor_data, P, k):
        ...
        # MC part
        Sp_mc = np.linalg.cholesky(P_xx)
        Sr_mc = np.linalg.cholesky(P_zz)
        ...
        while Evaluation > self.eps:
            ...
            x_new_mc = x_old_mc + K*(sensor_data-obs_mean)
            Evaluation = np.linalg.norm(x_new_mc - x_old_mc)/np.linalg.norm(x_old_mc)
        x_posterior = x_new_mc
        P_posterior = (np.eye(self.states_dimension)-K*H_mc)*P*(np.eye(self.states_dimension)-K*H_mc).T \
            + K*self.noise_R*K.T
