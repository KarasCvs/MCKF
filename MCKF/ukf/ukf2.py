...
class UKF():
    ...
    def ut_init(self, alpha=1e-3, beta=2, ki=0):
        ...
        # This is the different part, it causing calculation error.
        # and the line under it, is the mathod of paper <Major development under Gaussian filtering since unscented Kalman filter>
        # still, causing calculation error.
        self.W_cov[0] = self.W_mean[0] + (1 - self.alpha*self.alpha + self.beta)
        # for i in range(self.states_dimension):
        #     self.W_cov[i] = self.W_mean[i] + (1 - self.alpha*self.alpha + self.beta)
