...
if __name__ == "__main__":
    ...
    time_line, states, real_obs, sensor, ukf_states, ukf_MSE = ukf_sim.run()
    # time_line, states, real_obs, sensor = sim.system_only()
    ...
    _, _, _, _, mcukf_states, mcukf_MSE = mcukf_sim.run()
    # time_line, states, real_obs, sensor = sim.system_only()
    # Plot
    ...
    plt.plot(time_line, mcukf_MSE.A.reshape(N,), linewidth=1, linestyle="-", label="mcukf MSE")
    plt.grid(True)
    plt.legend(loc='upper left')
    plt.title("MSE")
    plt.figure(3)
